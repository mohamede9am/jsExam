...
async function searchByName(a) {
    let nameSearch = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${a}`);
    let nnn = await nameSearch.json();
    console.log(nnn);
    return nnn.meals;
}

async function displaySearch(a) {
    const ser = await searchByName(a);
    let mealDetails = '';
    for (let i = 0; i < ser.length; i++) {
        const number = await ser[i].idMeal;
        mealDetails += `<div class="col-md-3">
            <div class="meal position-relative overflow-hidden rounded-3 cursor-pointer" onclick="displayCat3('${number}')" id="${ser[i].idMeal}">
                <img class="w-100" src="${ser[i].strMealThumb}">
                <div class="meal-layer position-absolute d-flex align-items-center text-black p-2">
                    <h4 class="text-danger">${ser[i].strMeal}</h4>
                </div>
            </div>
        </div>`;
    }
    ...
    document.querySelector("#home .searchrow").innerHTML = mealDetails;
}
